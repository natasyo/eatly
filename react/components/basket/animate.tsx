  ...
  return (
    <>
      <style>
        ...
        60% {   
              transform: translate(${position.x * 0.9}px, ${position.y * 0.9}px) scale(40%);
              opasity:1;
          }
        99% {   
      ...
      <div
        ref={animateRef}
        className={`${className ? className : ''} absolute top-0 ${isAnimate ? 'z-50' : ''}`}
      >
        <div style={{ animation: `${isAnimate ? `animate${dishe.id} 1s linear .1s` : ''}` }}>
          {children}
        </div>
      </div>
    </>
